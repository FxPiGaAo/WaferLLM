#!/usr/bin/env python3
"""
Parallel MeshGEMV WSE-3 Runner

This script runs multiple MeshGEMV executions in parallel on WSE-3 hardware using a pool of worker processes.
It reads parameter sets from an input file and executes run_wse3.sh for each set.

Features:
  - Parallel execution using multiprocessing
  - Smart skip: automatically skips already-completed executions
  - Progress tracking with detailed output
  - Error handling and logging
  - WSE-3 hardware support
"""
...
def run_wse(params):
    """
    Run a single WSE-3 execution with the given parameters.

    Args:
        params: tuple of (P, M, N, group_num, output_folder, skip_check, preview_mode)

    Returns:
        tuple: (success, P, M, N, group_num, runtime, error_msg)
    """
    P, M, N, group_num, output_folder, skip_check, preview_mode = params

    output_file = os.path.join(output_folder, f"output_P{P}_M{M}_N{N}_G{group_num}.txt")

    # Check if output already exists and contains valid results (skip feature) - only if skip_check is False
    from datetime import datetime
    skip_timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    if not skip_check and os.path.exists(output_file):
        ...
        try:
            with open(output_file, 'r') as f:
                ...
                if "Mean" in content:
                    ...
                    if preview_mode:
                        ...
                        # Continue to run the execution below
            ...
            if preview_mode:
                ...
                # Continue to run the execution below
    elif not skip_check and not os.path.exists(output_file):
        # File doesn't exist - need to run
        if preview_mode:
            print(f"[{skip_timestamp}] ▶ Will run: P={P} M={M} N={N} group_num={group_num} (no output file found)")
            return (False, P, M, N, group_num, 0, "will_run")
        # If not preview mode, continue to run the execution below

    # If in preview mode, we should have already returned by now
    # This handles the case when skip_check is True and preview_mode is True
    if preview_mode:
        if skip_check:
            # If skip_check is True, we will run everything
            print(f"[{skip_timestamp}] ▶ Will run: P={P} M={M} N={N} group_num={group_num} (skip check disabled)")
            return (False, P, M, N, group_num, 0, "will_run")
        else:
            # Should not reach here - but just in case
            return (True, P, M, N, group_num, 0, "preview_done")

    # Run on WSE-3 (false = real device)
    cmd = f"./run_wse3.sh {P} {M} {N} {group_num} false"
    ...
    try:
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=7200  # 2 hour timeout (WSE runs can take longer)
        )
...
def main():
    parser = argparse.ArgumentParser(
        description='Run MeshGEMV executions in parallel on WSE-3',
...
Examples:
  ...
  # Run with custom max params (e.g., 500)
  python parallel_wse.py --max-params 500

  # Preview what will run or be skipped
  python parallel_wse.py --preview

  # Force re-run all parameters (skip the executed check)
  ...
  python parallel_wse.py -i params.txt -o results -n 4 --max-params 1000 --skip-check
    ...
    parser.add_argument('input_file', nargs='?', default='wse_in_out/input/input.txt',
                        help='Input file with parameters (default: wse_in_out/input/input.txt)')
    parser.add_argument('output_folder', nargs='?', default='wse_in_out/outputs',
                        help='Output folder for results (default: wse_in_out/outputs)')
    parser.add_argument('num_processes', nargs='?', type=int, default=1,
                        help='Number of parallel processes (default: 1 for testing)')
    ...
    parser.add_argument('--max-params', type=int, default=2000,
                        help='Maximum number of input parameters to execute (default: 2000)')
    parser.add_argument('--preview', action='store_true',
                        help='Preview mode: show what would be run or skipped without actually executing')
    parser.add_argument('--skip-check', action='store_true',
                        help='Skip the executed check and run all parameters (default: False, which means already executed parameters will be skipped)')
        ...
        for line in f:
            ...
            if line and not line.startswith('#'):
                ...
                if len(parts) == 4:
                    ...
                    params_list.append((P, M, N, group_num, output_folder, args.skip_check, args.preview))
    ...
    # Otherwise, limit to max_params
    elif len(params_list) > args.max_params:
        print(f"Limiting to first {args.max_params} parameters (--max-params={args.max_params})")
        params_list = params_list[:args.max_params]

    print(f"Found {len(params_list)} WSE execution(s) to {'preview' if args.preview else 'run'}")
    if args.preview:
        print(f"Preview mode enabled - no executions will be performed")
    print(f"Using {num_processes} parallel processes")
    print(f"Skip check: {'Disabled (will re-run all)' if args.skip_check else 'Enabled (will skip already executed)'}")
    print(f"Output folder: {output_folder}")
    print("-" * 80)

    # Run WSE executions in parallel (or preview)
    start_time = time.time()
    ...
    with Pool(processes=num_processes) as pool:
        ...
        for result in pool.imap_unordered(run_wse, params_list):
    ...
    if args.preview:
        ...
        print("Summary:")
        print(f"Total: {total} WSE executions")
