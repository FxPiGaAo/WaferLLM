#!/usr/bin/env python3
"""
Collect MeshGEMM simulation/WSE results from output folder into JSON format.
...
def parse_output_file(filepath):
    """
    Parse a single output file to extract P, M, K, N and mean cycle count.

    Args:
        filepath: Path to the output file

    Returns:
        dict with keys: P, M, K, N, mean_cycle_count
        None if parsing fails
    """
    # Extract parameters from filename (e.g., output_P64_M1024_K1024_N1024.txt)
    filename = os.path.basename(filepath)
    match = re.match(r'output_P(\d+)_M(\d+)_K(\d+)_N(\d+)\.txt', filename)

    if not match:
        print(f"Warning: Cannot parse filename {filename}")
        return None

    P, M, K, N = map(int, match.groups())
...
def collect_results(output_folder, json_output_file):
    ...
    output_files = list(output_folder.glob('output_P*_M*_K*_N*.txt'))
...
def main():
    parser = argparse.ArgumentParser(
        description='Collect MeshGEMM simulation/WSE results from output folder into JSON format.',
