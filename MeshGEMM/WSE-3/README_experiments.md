# MeshGEMM Parallel Experiment Runner

This directory contains scripts to run multiple MeshGEMM experiments in parallel, either on the simulator or WSE-3 hardware.

## Overview

MeshGEMM computes general matrix multiplication: `[M,N] = [M,K] @ [K,N]` on a `P×P` PE grid.
...
```bash
# Step 1: Generate parameter sets (using defaults: P=16-128 step 16, M=K=N=1024)
...
# WSE mode: M, K, N automatically generated as multiples of P
...
- `M_start`, `M_end`, `M_step`: Range for M (rows in first matrix) - Default: 1024, 1024, 1024
- `K_start`, `K_end`, `K_step`: Range for K (shared dimension) - Default: 1024, 1024, 1024
- `N_start`, `N_end`, `N_step`: Range for N (columns in second matrix) - Default: 1024, 1024, 1024

**WSE Mode:**
Automatically generates M, K, N as multiples of P:
...
- **M, K, N**: For each P value, automatically iterate through **P, 2P, 3P, 8P**
  - This generates 4×4×4 = 64 parameter combinations per P value
  - Example: For P=16, generates combinations like (16,16,16), (16,32,16), (16,48,48), etc.
...
```bash
# Generate with defaults (P: 16-730 step 64, M/K/N = P,2P,3P,8P)
# This generates 12 P values × 64 combinations = 768 parameter sets
python generate_params.py --wse

# Custom P range (P: 16-256 step 64)
# This generates 4 P values × 64 combinations = 256 parameter sets
python generate_params.py --wse wse_params.txt 16 256 64

# Smaller range for testing (P: 64-128 step 64)
# This generates 2 P values × 64 combinations = 128 parameter sets
...
# K range: 1024 to 1024 (step: 1024)
...
# M, K, N: For each P, iterate through P, 2P, 3P, 8P
# Format: P M K N
# MeshGEMM computes [M,N] = [M,K] @ [K,N] on P×P PE grid
#
16 16 16 16
...
Each simulation creates an output file named `output_P{p}_M{m}_K{k}_N{n}.txt` containing:
...
--------------------------------------------------------------------------------
Running: ./run_sim.sh 16 1024 1024 1024 -> simulator_in_out/outputs/output_P16_M1024_K1024_N1024.txt
✓ Completed: P=16 M=1024 K=1024 N=1024 (Runtime: 45.23s)
⊘ Skipped: P=32 M=1024 K=1024 N=1024 (already ran - output file exists)
Running: ./run_sim.sh 48 1024 1024 1024 -> simulator_in_out/outputs/output_P48_M1024_K1024_N1024.txt
✓ Completed: P=48 M=1024 K=1024 N=1024 (Runtime: 52.18s)
--------------------------------------------------------------------------------
Summary:
Total: 8 simulations
Successful: 8
...
Runtimes:
  P=16 M=1024 K=1024 N=1024: 45.23s
  P=48 M=1024 K=1024 N=1024: 52.18s
```
...
Same as simulator mode - automatically skips executions with existing output files.
...
# Full specification using flags
python parallel_wse.py -i params.txt -o results -n 4
...
Each WSE execution creates an output file named `output_P{p}_M{m}_K{k}_N{n}.txt` with the same format as simulator mode.
...
# P: 16-64 step 16, M: 512-2048 step 512, K=N=1024
...
```bash
# Generate WSE parameters with defaults (P: 16-730 step 64, M/K/N = P,2P,3P,8P)
# This creates 768 parameter combinations
python generate_params.py --wse

# Run on WSE with 2 parallel processes
python parallel_wse.py -n 2
...
# This creates 256 parameter combinations (4 P values × 64 combinations)
python generate_params.py --wse wse_params.txt 16 256 64
...
```bash
rm simulator_in_out/outputs/output_P16_M1024_K1024_N1024.txt
```
...
tail -f simulator_in_out/outputs/output_P16_M1024_K1024_N1024.txt
...
│   │   ├── output_P16_M1024_K1024_N1024.txt
│   │   ├── output_P32_M1024_K1024_N1024.txt
│   │   └── ...
...
## Differences from Shift Experiments

The MeshGEMM experiment scripts are adapted from the Shift experiment scripts with these key differences:

1. **Parameters**: Uses `P M K N` instead of `P L M N`
   - MeshGEMM: `[M,N] = [M,K] @ [K,N]` on `P×P` PE grid
   - Shift: Different operation with different parameter meanings

2. **Parameter Generation Modes**:
   - **Simulator Mode**: Full user control over all parameter ranges (P, M, K, N independent)
   - **WSE Mode**: M, K, N automatically generated as multiples of P (P, 2P, 3P, 8P)
     - Default P range: 16-730 step 64 (generates 768 parameter combinations)
     - Each P value generates 4×4×4 = 64 combinations
   - Shift: Had max product constraints and L+P < 740 constraint

3. **WSE Parameter Defaults**:
   - MeshGEMM WSE: P from 16 to 730, step 64 → 12 P values
   - Shift WSE: P sequence from 16 to 650 with variable steps

4. **File Naming**:
   - MeshGEMM: `output_P{p}_M{m}_K{k}_N{n}.txt`
   - Shift: `output_P{p}_L{l}_M{m}_N{n}.txt`

5. **Command Line Arguments**:
   - MeshGEMM WSE: `--wse [output_file] [P_start] [P_end] [P_step]`
   - MeshGEMM Simulator: Full 13 arguments for all parameters
   - Shift: Both modes used similar argument structure
...
- Reduce matrix dimensions (M, K, N)
